...
def to_single_band_array(image: ImageInput) -> np.ndarray:
    ...
    if arr.ndim == 2:
        ...
        first_is_ch = first in (1, 2, 3, 4)
        last_is_ch = last in (1, 2, 3, 4)

        if first_is_ch and not last_is_ch:
            # Unambiguous channel-first: e.g. (2, 128, 128) or (3, 128, 128)
            channels = np.moveaxis(arr, 0, -1)
        elif first_is_ch and last_is_ch and first != last:
            # Ambiguous but shape[0] looks like a channel count
            channels = np.moveaxis(arr, 0, -1)
        elif last_is_ch:
            # Channel-last: e.g. (128, 128, 2) or (128, 128, 3)
            channels = arr
        else:
            raise ValueError(
        ...
        if channels.shape[-1] == 1:
            ...
            # Dual-band thermal (B10 + B11) -> primary band or mean thermal
            single_band = channels[..., 0]
        else:
