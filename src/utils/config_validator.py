...
def validate_config(cfg: dict[str, Any]) -> list[str]:
    ...
    if seed is not None and not isinstance(seed, int):
        errors.append(
            f"project.seed must be an integer, got {type(seed).__name__}")
    ...
    if not isinstance(image_size, int) or image_size <= 0:
        errors.append(
            f"dataset.image_size must be a positive integer, got {image_size}")
    ...
    if not isinstance(input_channels, int) or input_channels <= 0:
        errors.append(
            f"dataset.input_channels must be a positive integer, got {input_channels}")

    output_channels = dataset.get("output_channels", 3)
    if not isinstance(output_channels, int) or output_channels <= 0:
        errors.append(
            f"dataset.output_channels must be a positive integer, got {output_channels}")
    ...
    if not isinstance(num_workers, int) or num_workers < 0:
        errors.append(
            f"dataset.num_workers must be a non-negative integer, got {num_workers}")
    ...
    if not isinstance(epochs, int) or epochs <= 0:
        errors.append(
            f"training.epochs must be a positive integer, got {epochs}")
    ...
    if not isinstance(decay_start, int) or decay_start < 0:
        errors.append(
            f"training.decay_start_epoch must be a non-negative integer, got {decay_start}")
    ...
    if resume_from and not Path(resume_from).exists():
        logger.warning(
            "training.resume_from points to non-existent file: %s", resume_from)

    # --- Model (optional) ---
    model = cfg.get("model", {})
    gen_impl = model.get("generator", {}).get("implementation")
    if gen_impl is not None and str(gen_impl).lower() not in (
        "hd",
        "resnet",
        "global_resnet",
        "hd_resnet",
    ...
    for key in ("best_checkpoint", "latest_checkpoint", "final_checkpoint"):
        ...
        if ckpt_path:
            ...
            try:
                ...
                errors.append(
                    f"paths.{key} parent directory cannot be created: {exc}")
