...
def download_file_resumable(url: str, dest_path: Path, max_retries: int = 10, chunk_size: int = 4 * 1024 * 1024):
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            print(f"  Downloading {dest_path.name} starting from {existing_bytes / 1e6:.1f} MB (Attempt {attempt}/{max_retries})...", flush=True)
                    ...
                    for chunk in r.iter_content(chunk_size=chunk_size):
                        if chunk:
                            ...
                            if total_size > 0:
                                ...
                                print(f"\r    {dest_path.name}: {downloaded / 1e6:.1f}/{total_size / 1e6:.1f} MB ({pct:.1f}%)", end="", flush=True)
                print()
