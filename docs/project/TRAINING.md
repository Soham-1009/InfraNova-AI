...
- **Decay**: Maintains a constant LR until `decay_start_epoch` (450), then linearly decays the LR to 0 over the remaining epochs (up to 500).
...
To start training on Kaggle (Recommended):
1. Navigate to your Kaggle Notebook (`notebooks/KAGGLE.ipynb`).
2. Ensure you have attached the `infranova-ai-full` dataset containing the `splits` folder.
3. Run the notebook cells, which will execute the training script with dual T4 GPUs.
