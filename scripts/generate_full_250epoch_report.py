...
def generate_report():
    ...
    print(
        f"Master telemetry saved to {master_csv} ({len(df_combined)} epochs: {df_combined['epoch'].min()} to {df_combined['epoch'].max()})"
    )
    ...
    _fig, axes = plt.subplots(2, 2, figsize=(14, 10), dpi=300)
    ...
    # MAE & RMSE
    ax = axes[1, 0]
    ax.plot(epochs, df_combined["val_mae"], color="#ea580c", lw=1.8, label="Validation MAE")
    ax.plot(epochs, df_combined["val_rmse"], color="#d97706", lw=1.8, label="Validation RMSE")
    ax.axvline(100, color="gray", linestyle="--", alpha=0.7)
    ax.set_title("Validation MAE & RMSE Progression", fontweight="bold")
    ax.set_ylabel("Error (Lower is Better)")
    ax.legend()

    # Spectral Angle Mapper (SAM)
    ax = axes[1, 1]
    ax.plot(epochs, df_combined["val_sam"], color="#0891b2", lw=1.8, label="Validation SAM (rad)")
    ax.axvline(100, color="gray", linestyle="--", alpha=0.7)
    ...
    _fig, axes = plt.subplots(1, 3, figsize=(18, 5), dpi=300)
    ...
    ax.axhline(
        df_combined["duration_sec"].mean(),
        color="#059669",
        linestyle="--",
        alpha=0.7,
        label=f"Mean ({df_combined['duration_sec'].mean():.1f}s)",
    )
    ...
    stats = {
        ...
        "stage_averages": {
            ...
            "stage2_epochs_101_200": {
                "mean_ssim": float(
                    df_combined[(df_combined["epoch"] > 100) & (df_combined["epoch"] <= 200)]["val_ssim"].mean()
                ),
                "mean_psnr": float(
                    df_combined[(df_combined["epoch"] > 100) & (df_combined["epoch"] <= 200)]["val_psnr"].mean()
                ),
                "mean_lab": float(
                    df_combined[(df_combined["epoch"] > 100) & (df_combined["epoch"] <= 200)]["val_lab_error"].mean()
                ),
                "mean_sat_ratio": float(
                    df_combined[(df_combined["epoch"] > 100) & (df_combined["epoch"] <= 200)]["val_sat_ratio"].mean()
                ),
                "sat_ratio_std": float(
                    df_combined[(df_combined["epoch"] > 100) & (df_combined["epoch"] <= 200)]["val_sat_ratio"].std()
                ),
            },
