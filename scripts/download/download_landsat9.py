    ...
    try:
        with rasterio.open(filepath) as src:
            if src.count != expected_bands:
                raise ValueError(f"Expected {expected_bands} bands, got {src.count}")
...
def process_region(
    region_id: str, region_info: dict, output_dir: Path, overwrite: bool = False, verbose: bool = False
) -> tuple[bool, str]:
    ...
    attempt = 0
    total_attempts = len(valid_radii) * len(SHIFTS) * len(DATE_RANGES) * len(CLOUD_COVERS)
            ...
            for date_start in DATE_RANGES:
                for cloud in CLOUD_COVERS:
                    ...
                    while current_radius <= 40000:
                        ...
                        if cloud < 100:
                            collection = collection.filter(ee.Filter.lt("CLOUD_COVER", cloud))

                        collection = collection.sort("CLOUD_COVER").sort("system:time_start", False)
...
def process_region_b11(
    ...
    url = img_toa.select(["B11"]).getDownloadURL(
        {"crs": target_crs, "region": geom, "dimensions": [w, h], "format": "GEO_TIFF"}
    )
...
def worker(task: tuple) -> tuple:
    ...
    for attempt in range(1, 6):  # Max 5 retries for network errors
        try:
            if band11_only:
                success, msg = process_region_b11(region_id, region_info, baseline_dir, output_dir, overwrite, verbose)
            else:
                success, msg = process_region(region_id, region_info, output_dir, overwrite, verbose)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Direct Landsat 9 Downloader (Local)")
    parser.add_argument(
        "--output-dir", type=str, default=str(PROJECT_ROOT / "data/landsat9/raw"), help="Output directory"
    )
    parser.add_argument(
        "--baseline-raw-dir",
        type=str,
        default=str(PROJECT_ROOT / "data/landsat9/raw"),
        help="Baseline raw directory containing tir.tif for scene matching",
    )
    parser.add_argument(
        "--band11-only",
        action="store_true",
    ...
    parser.add_argument("--verbose", action="store_true", help="Print every parameter combination attempted")
    ...
    state = {"completed": 0, "successful_regions": [], "failed_regions": [], "network_errors": []}
    ...
    def run_pass(regions_to_run, pass_name):
        ...
        tasks = [
            (rid, REGIONS[rid], output_dir, args.overwrite, args.verbose, args.band11_only, baseline_dir)
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=args.workers) as executor:
                ...
                for i, future in enumerate(concurrent.futures.as_completed(futures), 1):
                    ...
                    if success:
                        ...
                        if not args.verbose:
                            print(f"[{i}/{total}] [OK] {REGIONS[region_id]['name']}")
                    else:
                        if is_network:
                            if region_id not in state["network_errors"]:
                                state["network_errors"].append(region_id)
                            if not args.verbose:
                                print(f"[{i}/{total}] [NET] Network Error: {REGIONS[region_id]['name']} ({msg})")
                        else:
                            if region_id not in state["failed_regions"]:
                                state["failed_regions"].append(region_id)
                            if not args.verbose:
                                print(f"[{i}/{total}] [FAIL] Failed: {REGIONS[region_id]['name']} - {msg}")
    ...
    pass1_regions = [r for r in all_regions if r not in state["successful_regions"]]
    ...
    # Pass 2
    pass2_regions = state["failed_regions"].copy() + state["network_errors"].copy()
    if pass2_regions:
        run_pass(pass2_regions, "PASS 2")

    # Pass 3
    pass3_regions = state["failed_regions"].copy() + state["network_errors"].copy()
    if pass3_regions:
