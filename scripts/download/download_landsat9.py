...
'geyzing': {'lat': 27.2930, 'lon': 88.2570, 'name': 'Geyzing'},
...
def worker(task: tuple) -> tuple:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            success, msg = process_region(region_id, region_info, output_dir, overwrite)
            if success:
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default=str(PROJECT_ROOT / "data/landsat9/raw"), help="Output directory")
    parser.add_argument("--workers", type=int, default=4, help="Number of parallel downloads")
    parser.add_argument("--start-index", type=int, default=0, help="Start index of regions")
    parser.add_argument("--end-index", type=int, default=None, help="End index of regions")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            if success:
                ...
                failed_count += 1
                logger.error(f"Failed to download {region_id} after {attempts} attempts: {msg}")
